...
import com.hitv.android.uiversion2.process.KeyProcess;
...
public class GalleryViewPager extends ViewPager {
		...
		setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (adapter instanceof AbstractAdapter) {
					((AbstractAdapter) adapter).getPrimaryItem().performClick();
				}
			}
		});
		post(myRun);
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		...
		if(event.getAction() == KeyEvent.ACTION_UP && (event.getKeyCode() == KeyEvent.KEYCODE_ENTER || event.getKeyCode() == KeyEvent.KEYCODE_DPAD_CENTER)){
			
			new Thread(new Runnable() {
				@Override
				public void run() {
					KeyProcess.sendKeyDownUpSync(KeyEvent.KEYCODE_DPAD_DOWN);
				}
			}).start();
	...
	public void onViewDestroy() {
		...
		if (tempAdapter instanceof AbstractAdapter) {
			...
			/*
			 * List<View> list = new ArrayList<View>(); int count =
			 * getChildCount(); for(int i = 0; i < count; i++){
			 * list.add(getChildAt(i)); }
			 */
			removeAllViews();
			/*
			 * for(View view : list){ ImageLoaderOptions.recycleBitmap(view); }
			 */
		}
	}
}
